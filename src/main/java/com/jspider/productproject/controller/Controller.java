...
import com.jspider.productproject.response.UserResponse;
...
public class Controller {
	...
	@PostMapping("addUser")
	ResponseEntity<UserResponse> addUser(@RequestBody User user ){
		User dto=service.addUser(user);
		if(dto!=null) {
			return new ResponseEntity<UserResponse>(new UserResponse(false, "User Added", dto),HttpStatus.ACCEPTED);
		}
		
		return new ResponseEntity<UserResponse>(new UserResponse(true, "User not Added"),HttpStatus.BAD_REQUEST);
